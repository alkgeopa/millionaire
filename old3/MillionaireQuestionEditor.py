...
labelFrameRight = LabelFrame(frameRight, bg=defColor, padx=10, pady=10, text="Επιλογές")
...
class Table:
    ...
    def __init__(self, parent):
        ...
        self.tree.column("difficulty", anchor=NW, minwidth=50, width=50, stretch=YES)
    ...
    def initInsert(self, allQuestions: list[Document]):
        for i, question in enumerate(allQuestions):
            self.tree.insert(
                parent="",
                index=int(question.doc_id),
                iid=int(question.doc_id),
                values=(
                    str(question.doc_id),
...
class AnswerFrame:
    def __init__(
        ...
        self.label = Label(
            self.frame, text=text, anchor=anchor, padx=padx, width=width, bg=defColor
        )
...
class OptionFrame:
    ...
    def __init__(self, options=dict()):
        ...
        self.difficultyFrame = LabelFrame(self.frame, bg=defColor, text="Δυσκολία")
        ...
        self.answerFrame = LabelFrame(self.frame, text="Απαντήσεις", bg=defColor)
        self.answerFrame.pack(side=TOP, anchor=NW, fill=X)

        self.correct = AnswerFrame(
            parent=self.answerFrame,
            text="Σωστή",
            anchor="w",
            textBox=options["correct"],
            width=10,
        )
        ...
        self.saveBtn = Button(
            self.buttonGroup,
            text="Αποθήκευση" if OptionFrame.state["newEntry"] == True else "Ενημέρωση",
        ...
        self.deleteBtn = Button(
            self.buttonGroup, text="Διαγραφή", bg="#eaa", command=deleteButtonHandler
        )
